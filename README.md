...
|-------|-------------|----------|---------|
| `api_url` | URL of the InvisiRisk API | Yes for setup, No for cleanup | N/A |
| `app_token` | Authentication token for the InvisiRisk API | Yes for setup, No for cleanup | N/A |
...
| `scan_id` | Scan ID from the setup step. Optional even for cleanup - if not provided, a default value will be used. | No | `''` |
